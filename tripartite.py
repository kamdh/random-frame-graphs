...
def tripartite_regular_spectrum(xs, pA, pB, kAB, kAC, kBC, 
    ...
    def _yfun_root_real(y, z, alpha, kAB, kAC, kBA, kBC, kCA, kCB):
        ...
        yAB,yAC,yBA,yBC,yCA,yCB = y_evalj
...
def tripartite_regular_configuration_graph(n, pA, pB, kAB, kAC, kBC,
    ...
    def _extract_blocks(A, n1, n2):
        ...
        Xt = A[n1:n1+n2, 0:n1]
        return X, Xt
    g = nx.bipartite_configuration_model([kAB]*nA, [kBA]*nB)
    X,Xt = _extract_blocks(nx.to_numpy_matrix(g), nA, nB)
    g = nx.bipartite_configuration_model([kAC]*nA, [kCA]*nC)
    Y,Yt = _extract_blocks(nx.to_numpy_matrix(g), nA, nC)
    g = nx.bipartite_configuration_model([kBC]*nB, [kCB]*nC)
    Z,Zt = _extract_blocks(nx.to_numpy_matrix(g), nB, nC)
    A = np.bmat([[np.zeros((nA,nA)), X, Y],
                 [Xt, np.zeros((nB,nB)), Z],
